...
class HAPIDistributor:
    ...
    def assign_proxy(self, task_type):
        ...
        # Prioriza proxies com melhor performance
        ...
        selected = next(
            (p for p in sorted_proxies 
             if self.current_load.get(p, 0) < 3),  # Máx 3 tarefas/proxy
            random.choice(sorted_proxies)
        )
    ...
    def _refresh_proxy_pool(self):
        """Atualiza pool de proxies ativos"""
        self.proxy_pool = deque([
            "194.32.78.1:8080",
            "185.243.56.2:3128",
            "201.166.34.3:8080"
